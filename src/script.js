...
function setupUI() {
    ...
    if (titleApi) titleApi.textContent = settings.name || "HanzoMarket API";
    if (descApi) descApi.textContent = settings.description || "Interactive API documentation with real-time testing";
    if (footer) footer.textContent = `© ${new Date().getFullYear()} ${settings.creator || "HanzoMarket"} - ${settings.name || "HanzoMarket API"}`;
...
async function executeRequest(event, catIndex, endpointIndex, method, path, produces) {
    ...
    try {
        ...
        const response = await fetch(url, {
            ...
            headers: {
                'Accept': '*/*',
                'User-Agent': 'HanzoMarket-API-Docs'
            }
        });
